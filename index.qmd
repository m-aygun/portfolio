...
set.seed(5)
train <- sample(c(TRUE, FALSE), nrow(fulldata5), replace = TRUE)
...
After this the model is fitted to training data using linear multiple regression, and this is the coeeficients of the model.
...
I will now test the model on new data, the testdata. Validation method used is 5-fold CV due to the balance between bias and variance. After this the MSE will be calculated
...
The prediction has been transformed into a Shiny app that can be implemented in the VFF system.

#Conclusion

The mode requires more work. The model is build only on publicly available data. In order to improve the prediction for more useful results it the model should be updated with intern data. this could be variables such as markting data, events in the local society, ticket prices etc.
