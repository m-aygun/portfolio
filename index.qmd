...
api_key <- "&api-key=7d5db05b-3c27-4b2b-ac78-728ddd02a9aa" #Husk at indsætte egen DMI
...
After importing the data the data has been cleaned and quantitative and qualitative variabels has been created

### Modeling

### Explorative analysis

In order to determin the significance of the qualitative variables I perform a corelationanalysis using a corelation heatmap. I wish to determine if there is any correlation bwtween the quantitative variables and Y (Tilskuere)
...
Her we can determin that there is a correlation between Indbyggertal and Synlighed. Therefore we will now check for correlation between these two variables. I am filtering out the non relevant variables as well.
...
I will not check the significance of each variable. Here we can se that the significant variables are: Superliga, ferie, Dagtype, Tid på dagen og Synlighed, and I chose to continue with these.
...
fulldata5 <- fulldata4 %>% dplyr::select("Tilskuere", "Superliga/1.Division", "Ferieuger", "Modstander Type", "Dagtype", "Synlighed")
```

In order to model the data I will make training data and test data.

```{r}
dim(fulldata4)

# Laver en logist vektor, hvis length er lig med antallet af 
# observationer i fulldata (235). sample-funktionen genererer 
# tilfældige TRUE og FALSE værdier. Der genereres cirka lige 
# mange TRUE og FALSE - værdier:

# Bemærk at nrow(fulldata2) = 235


set.seed(5)
train <- sample(c(TRUE, FALSE), nrow(fulldata4), replace = TRUE)


# På den måde fordeles observationerne i Hitters til enten et 
# træningssæt (TRUE) eller til et testsæt (FALSE):

# Det modsatte af train er !train og dermed dannes testsættet:
test <- (!train)

# Bemærk at de er gensidig udelukkende:
...
After this the model is fitted to training data using linear multiple regression, and this is the coeeficients of out model.

```{r}

lm.fit1 <- lm(Tilskuere ~ ., data = fulldata5[train, ])
...
After this I am checking for outliers using studentized residuals.

```{r}

residuals <- residuals(lm.fit1)

# Beregn standardfejl for residuals (R's default er studentized residuals)
residuals_se <- sqrt(sum(residuals^2) / (length(residuals) - 2))

# Beregn studentized residuals
studentized_residuals <- residuals / residuals_se

# Opret et scatterplot af studentized residuals
ggplot(data = NULL, aes(x = seq_along(studentized_residuals), y = studentized_residuals)) +
  geom_point() +
  labs(x = "Observationer", y = "Studentized Residuals") +
  ggtitle("Scatterplot af Studentized Residuals")

# Find outliers (|studentized_residuals| > 3)
outliers <- abs(studentized_residuals) > 2 | abs(studentized_residuals) < -2

# Udskriv de outliers
outlier_indices <- which(outliers)
outlier_values <- studentized_residuals[outliers]

outlier_data <- data.frame(Outlier_Index = outlier_indices, Studentized_Residual = outlier_values)
print(outlier_data)

outlier_data
```

I wil now test the model on new data, the testdata

```{r}

# first the Y and X variables are defined

x <- model.matrix(Tilskuere ~ ., fulldata5)[, -1]
y <- fulldata5$Tilskuere
