...
format:
  ...
  pdf:fd<az    theme: flatly
    toc: true
    number-sections: true
---

![Soccerplayer data](DALL·E 2024-01-20 08.21.21 - Create an image of a soccer player in a dynamic pose, looking upwards towards a soccer ball suspended in the air. The player is surrounded by futurist.png){fig-alt="Soccerplayer data"}
...
Choosing Optimal Lambda (or Alpha):
For each combination of lambda values (both for Lasso and Ridge), the model is trained on k-1 folds and validated on the remaining fold. The average MSE across all folds for each lambda value is calculated.
...
The best performing model is here shown as the linear regression, since ethe MSE is lowest. o I am choosing to continue with this model.
